...
ap.add_argument("-n", "--name", required=True, help="Complex")
...
logging.basicConfig(
    filename=complx + '-ti-quicker-analyze.log',
...
print(complx)
if os.path.isfile(complx + '-ti-quicker-more.log'):
    print('Skipping...')
    sys.exit()
...
structure = pt.load(
    os.path.join(complx, 'AMBER', 'APR', 'windows', 'a000', 'solvate.rst7'),
...
for fraction_index, fraction in enumerate(fractions):
    ...
    data[fraction_index, 1] = fecalc.results['attach']['ti-block']['fe']
    data[fraction_index, 2] = fecalc.results['attach']['ti-block']['sem']
    data[fraction_index, 3] = fecalc.results['attach']['ti-block']['n_frames']

    data[fraction_index, 4] = fecalc.results['pull']['ti-block']['fe']
    data[fraction_index, 5] = fecalc.results['pull']['ti-block']['sem']
    data[fraction_index, 6] = fecalc.results['pull']['ti-block']['n_frames']
...
np.savetxt('{}-ti-quicker-more.log'.format(complx), data)
